...
const HomeCategory = () => {
  ...
  return (
    <section className='lg:px-5'>
       <h1 className='text-[40px] md:text-[30px] sm:text-[25px] overflow-hidden py-5 relative font-semibold text-maincolor after:h-[2px] after:w-full after:bg-text2 after:absolute after:ml-[30px] after:top-[58%]'>Danh mục món ăn</h1>
       ...
       <Link to={`/thuc-don/${item.menuType}`} key={item._id}>
        ...
        <div className='relative pt-[100%] md:pt-[60%] overflow-hidden rounded-t-[10px]'>
          <img className='absolute top-0 left-0 h-full w-full object-cover rounded-t-[10px] hover:scale-105 duration-500' src={item.imgMenu} alt={item.menuType} loading='lazy'/>
      </div>
